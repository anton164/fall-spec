...
df = pd.read_json(INPUT_DATA_LOCATION + args.input_file)
...
df_continuous.to_csv(f"{OUTPUT_DATA_LOCATION}{args.output_name}numeric.txt", index=False, header=False)
df_symbolic.to_csv(f"{OUTPUT_DATA_LOCATION}{args.output_name}categ.txt", index=False, header=False)
df_label.to_csv(f"{OUTPUT_DATA_LOCATION}{args.output_name}_label.txt", index=False, header=False)
df.loc[:,'created_at'].to_csv(f"{OUTPUT_DATA_LOCATION}{args.output_name}time.txt", index=False, header=False)

text_file = open(f"{OUTPUT_DATA_LOCATION}{args.output_name}numeric.txt", "w")
df_symbolic.shape[0]
n = text_file.write('\n'*df_symbolic.shape[0])
text_file.close()